...
def index():
    ...
    posts = [
        {'author': {'username': 'Rachel'},
         'body': 'Starting my own blog'},
        {'author': {'username': 'Rachel'},
         'body': 'testing my own blog'}]
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Herzlichen Glueckwunsch! Du bist jetzt angemeldet.')
        return redirect(url_for('login'))
...
@login_required
def protected():
    return render_template('protected.html', title='Gesichert')